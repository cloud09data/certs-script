...
send_email() {
    local body="$1"
    echo "$body" | mail -s "$email_subject" -a "Content-Type: text/html" "$email_recipient"
}
...
# Function to format the email body as an HTML table
format_email_body() {
    local table_content="$1"
    local html_body="<html><body><h2>SSL Certificate Expiry Notification</h2>"
    html_body+="<table border='1' cellpadding='5'><tr><th>Hostname</th><th>Installed Date</th><th>Expiration Date</th></tr>"
    html_body+="$table_content</table></body></html>"
    echo "$html_body"
}
    ...
    if [ -f "$server/jboss/standalone/configuration/keystore.jks" ]; then
        ...
        if [ -n "$cert_dates" ]; then
            ...
            if [ "$expiry_days" -lt 0 ]; then
                color="<font color='red'>$hostname</font>"
            elif [ "$expiry_days" -lt "${warning_periods[1]}" ]; then
                color="<font color='orange'>$hostname</font>"
            else
                color="$hostname"
            fi
            table_content+="<tr><td>$color</td><td>$installed_date</td><td>$expiry_date</td></tr>"
        fi
    fi
...
# Format email body as an HTML table
html_body=$(format_email_body "$table_content")
